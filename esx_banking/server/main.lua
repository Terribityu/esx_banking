...
ESX.RegisterServerCallback("esx_banking:getPlayerData1", function(source, cb, data)
    local xPlayer = ESX.GetPlayerFromIdentifier(data)
    yPlayer = ESX.GetPlayerFromIdentifier(data)
...
RegisterServerEvent('esx_banking:requestCard')
AddEventHandler('esx_banking:requestCard', function()
    local xPlayer = ESX.GetPlayerFromId(source)

    if xPlayer then
        local xIdentifier = xPlayer.getIdentifier() -- This gets the player's identifier
        local fullName = xPlayer.getName() -- This gets the player's full name

        if xPlayer.getInventoryItem('mastercard').count < 1 then
            xPlayer.addInventoryItem('mastercard', 1, {identifier = xIdentifier, description = fullName})
            TriggerClientEvent("esx:showNotification", source, TranslateCap("requestcard"), "success")
        else
            TriggerClientEvent("esx:showNotification", source, TranslateCap("cardexist"), "error")
        end
    end
end)
