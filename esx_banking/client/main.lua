...
RegisterNUICallback('requestCard', function(data, cb)
    TriggerServerEvent('esx_banking:requestCard')
end)
...
RegisterNUICallback('checkPincode', function(data, cb)
    ...
    ESX.TriggerServerCallback("esx_banking:checkPincode", function(pincode)
        ...
        if pincode then
            ...
            ESX.ShowNotification(TranslateCap('account_is_locked') .. " (" .. trypassword + 1 ..  "Tries)", "error")
        else
            cb({
                error = true
            })
...
function BANK:openATM(atm,identifier)
    state = true
    atm = atm or false
    SetNuiFocus(state, state)
    inMenu = state
    ...
    ESX.TriggerServerCallback('esx_banking:getPlayerData1', function(data)
        SendNUIMessage({
            showMenu = true,
            openATM = atm,
            datas = {
...
RegisterNetEvent('esx_banking:usecard')
AddEventHandler('esx_banking:usecard', function(item, wait, cb)
    ...
    for i = 1, #Config.AtmModels do
        local atm = GetClosestObjectOfType(playerCoords.x, playerCoords.y, playerCoords.z, 1.0, Config.AtmModels[i], false, false, false)
